...
app.get('/quotes', async (_req, res) => {
  try {
    const [btc, eth] = await Promise.all([
      fetch('https://pi.coinbase.com/v2/exchange-rates?currency=BTC', { timeout: 5000 }).then(r => r.json()),
      fetch('https://pi.coinbase.com/v2/exchange-rates?currency=ETH', { timeout: 5000 }).then(r => r.json())
    ]);
